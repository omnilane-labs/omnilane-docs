...
The `cumulative` value is the all-time cumulative amount deposited for this LP and token address. It is later used by the user to provide a `witness` of this value, fueling the Omnilane `Cumulative State Consensus` mechanism.
